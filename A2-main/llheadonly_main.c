...
{
    // Initalizing an Empty Linked List
	LLvNode* HeadLL = NULL;
	FILE *fp;
	FASTArecord fRecord;
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			LLvNode* createNewNode;
			createNewNode = llNewNode(fRecord.description, fRecord.sequence);
			if(HeadLL == NULL)
			{
				HeadLL = createNewNode;
				
			}
			else
			{
				HeadLL = llAppend(HeadLL,createNewNode);
			}
