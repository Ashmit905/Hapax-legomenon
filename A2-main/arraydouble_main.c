...
{
	FASTArecord *doubleRecord = malloc(sizeof(FASTArecord));
	int fastRecordSizeNew = 1;
	...
	do
	{
		...
		else if (status > 0)
		{
			...
			// Storing my array and allocating memory in same line
			// FASTArecord *doubleRecord = (FASTArecord *)malloc(sizeof(FASTArecord));

			// Firstly Check if Memory Allocation was Successful
			...
			// When Resizing the Record, double the size by 2
			if (fastRecordSizeNew == recordNumber)
			{
				doubleRecord = realloc(doubleRecord, sizeof(FASTArecord) * (fastRecordSizeNew * 2));
				fastRecordSizeNew = fastRecordSizeNew * 2;
			}

			// Store data into this array that we have just created
			doubleRecord[recordNumber] = fRecord;
	...
	printf(" %d FASTA records -- %d allocated (%.3lf%% waste)\n", recordNumber, fastRecordSizeNew, ((fastRecordSizeNew - recordNumber) * 100.0) / fastRecordSizeNew);
